...
class StaticCSSParser(BaseParser):
    ...
    async def parse(self, task: ScrapingTask) -> Dict[str, Any]:
        ...
        # Solicitamos el contexto de socket seguro a la infraestructura inyectada
        async with self.network_client.create_session() as session:
            self.log.info(f"Infiltración TLS activa. Descargando: {task.url} | Tarea ID: {task.task_id}")
            
            try:
                response = await session.get(str(task.url), allow_redirects=True)
                
                if response.status_code == 404:
                    raise ScrapingError(ErrorCategory.NOT_FOUND, "Página no existe", task.task_id)
                if response.status_code in [403, 429]:
                    raise ScrapingError(ErrorCategory.BLOCKED, "IP bloqueada o Rate Limit", task.task_id)
                if response.status_code >= 500:
                    raise ScrapingError(ErrorCategory.SERVER_ERROR, f"Fallo del servidor destino: {response.status_code}", task.task_id)
