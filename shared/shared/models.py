...
class ScrapingTask(BaseModel):
    ...
    task_id: str = Field(default_factory=lambda: str(uuid4()), description="ID único de la tarea para deduplicación")
    job_id: str = Field(..., description="ID del grupo de scraping (ej: amazon-beauty-2026)")
    
    # Objetivo
    url: HttpUrl = Field(..., description="URL validada a extraer")
    
    # Configuración del Motor (Dinámico)
    parser_type: str = Field(..., description="Determina el script o lógica a usar (ej: 'static_css', 'playwright_amazon')")
    ...
    depth: int = Field(default=0, ge=0, description="Nivel de profundidad para modo recursivo")
    max_depth: int = Field(default=3, ge=0, description="Límite para evitar bucles infinitos")
    
    # Resiliencia
    retry_count: int = Field(default=0, description="Contador de reintentos realizados")
    max_retries: int = Field(default=3, description="Máximo de intentos antes de ir a la DLQ")
